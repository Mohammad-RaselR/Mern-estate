...
export default function CreateListing() {
    ...
    const handleSubmit= async(e)=>{
        ...
        try{
            ...
            if(+formData.discountPrice > +formData.regularPrice) return setError('Discount price must be less than regular price');
            setLoading(true); 
            setError(false); 
      ...
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4">
        <div  className="flex flex-col gap-4 flex-1 ">
          ...
          <textarea
            ...
            className="p-3 border rounded-lg" value={formData.description}
          ></textarea>
          <input
            type="text"
            placeholder="Address"
            ...
            <p className="font-semibold">Images:
                <span className="font-normal text-gray-600 ml-2">The first image will be the cover (max 6)</span>
                 ...
                 <div className="flex gap-4 ">
                    ...
                    <button disabled={uploading} onClick={handleImageSubmit} className="p-3 text-green-700 border border-green-700 rounded uppercase hover:shadow-lg disabled:opacity-80">
                 ...
                 <p className="text-red-700 text-sm">{imageError && imageError }</p>
                 {
                    formData.imageUrl.length>0 && formData.imageUrl.map((url, index)=>( 
                        <div key={url} className="flex justify-between p-3 border items-center">
                            ...
                            <button onClick={()=>handleRemoveImage(index)} className="p-3 text-red-700 hover:opacity-75 rounded-lg uppercase" >Delete</button>
                        </div>
                    ))
                 }
